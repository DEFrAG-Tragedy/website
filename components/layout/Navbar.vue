...
import WCSearch from "~/components/webcomponents/WCSearch.vue";
...
const isXmlRenderPage = ref(route.name === 'xml-render' || route.name === 'search' || route.name === 'documents');

const { showResults } = storeToRefs(appStore);

watch(route, (newRoute) => {
  isHomePage.value = newRoute.name === 'index' || newRoute.name === 'search-page';
  isXmlRenderPage.value = newRoute.name === 'xml-render' || newRoute.name === 'search' || newRoute.name === 'documents';
});
